...
RSpec.describe "Tasks filtering", type: :request do
  ...
  let!(:scheduled_in_range_2) { create(:task, status: :scheduled, scheduled_date: Date.today + 7.day) }
  ...
  describe "filtering by status" do
    it "returns tasks with specified status for the week" do
      get "/api/v1/tasks", params: { status: "scheduled" }

      body = JSON.parse(response.body)
      ids = body.map { |task| task["id"] }
      expect(ids).to contain_exactly(scheduled_in_range.id, scheduled_in_range_2.id)
    end
  end
  ...
  describe "filtering by date range" do
    it "returns tasks within date range" do
      ...
      body = JSON.parse(response.body)
      ids = body.map { |task| task["id"] }
      expect(ids).to contain_exactly(scheduled_in_range.id, scheduled_in_range_2.id, completed_in_range.id)
    end
  end
